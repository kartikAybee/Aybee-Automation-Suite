...
public class PreviewJourneyPage extends BasePage {
    ...
    private void answerDemographicQuestion(String optionValue, SoftAssert sa) {
        ...
        // Re-find the option by locator on EACH attempt and JS-click it — never hold a WebElement
        // reference across a wait, since Bubble.io re-renders reactively and would make it stale.
        boolean optionClicked = false;
        for (int attempt = 0; attempt < 3 && !optionClicked; attempt++) {
            try {
                new WebDriverWait(driver, 15).until(ExpectedConditions.elementToBeClickable(anyOption));
                new WebDriverWait(driver, 15).until(ExpectedConditions.elementToBeClickable(optionLocator));
                ...
                // Bubble re-rendered between locate and click — loop and re-find.
            } catch (Exception e) {
                break;
            }
        }
