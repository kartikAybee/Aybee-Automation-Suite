...
public class ShopSetupPage extends BasePage {
    ...
    private final By productDescriptionField = By.id("edit_product_description_scenario_Scenario A");
    ...
    public ShopSetupPage clickGoAndWaitForProducts() {
        ...
        // Explicitly confirm Scenario A's product card image has loaded.
    ...
    private String capturedScenarioBPrice;
    ...
    @Step("Paste product description into Scenario A description field")
    public ShopSetupPage setProductDescription(String description) {
        WebElement input = wait.until(ExpectedConditions.visibilityOfElementLocated(productDescriptionField));
        input.clear();
    ...
    // Re-fills both fields with the intended Scenario B values immediately before saving.
    // Bubble.io reactive events (triggered by image deletion, price entry, etc.) can silently
    // clear fields. Rather than checking whether they are empty, we unconditionally restore
    // them from the values captured when Scenario A was first read, then apply the B change.
    @Step("Re-fill Scenario B name and price fields immediately before saving")
    private void validateAndFixFieldsBeforeSave() {
        blurActiveElement();
        ...
        if (capturedScenarioBName != null && !capturedScenarioBName.isEmpty()) {
            for (int attempt = 1; attempt <= 3; attempt++) {
                try {
                    ...
                    try { Thread.sleep(500); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                    String current = driver.findElement(productNameField).getAttribute("value");
                    if (capturedScenarioBName.equals(current)) {
                        System.out.println("[ShopSetup] Scenario B name confirmed before save: " + current);
                        break;
                    }
                    System.out.println("[ShopSetup] Scenario B name cleared after fill (attempt " + attempt + ") — retrying");
                } catch (Exception e) {
                    System.out.println("[ShopSetup] Could not re-fill Scenario B name (attempt " + attempt + "): " + e.getMessage());
                }
            }
        }

        if (capturedScenarioBPrice != null && !capturedScenarioBPrice.isEmpty()) {
            try {
                WebElement priceInput = wait.until(
                    ExpectedConditions.presenceOfElementLocated(productPriceField));
                priceInput.click();
                priceInput.click();
                priceInput.click();
                priceInput.sendKeys(capturedScenarioBPrice);
                System.out.println("[ShopSetup] Re-filled Scenario B price before save: " + capturedScenarioBPrice);
            } catch (Exception e) {
                System.out.println("[ShopSetup] Could not re-fill Scenario B price before save: " + e.getMessage());
            }
        }
    ...
    public ShopSetupPage saveChanges() {
        // Guard: verify both name and price are non-empty before committing.
        // Name is re-typed if cleared; price throws immediately if blank.
        validateAndFixFieldsBeforeSave();
    ...
    // must be non-empty before the caller proceeds with edits.
    ...
    // Scrolls the Next button into the viewport before clicking — it may be below
