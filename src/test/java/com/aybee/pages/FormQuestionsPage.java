...
public class FormQuestionsPage extends BasePage {
    ...
    // Reloads the Form Questions editor once, right before opening preview, so the backend creatives /
    // products load fully (they can render incompletely in the preview journey on the very first open).
    // Mirrors the PDP precaution. The caller commits/validates fields BEFORE this reload.
    ...
    public String clickPreviewAndGetUrl(int[] retriggerQueue) {
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            ...
            if (toastDetectedDuringPoll) {
                ...
                // No toast → the chooser stays open, so close it before retrying.
