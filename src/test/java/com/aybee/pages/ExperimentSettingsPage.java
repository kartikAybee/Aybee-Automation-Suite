...
public class ExperimentSettingsPage extends BasePage {
    ...
    // Adds all 5 business questions by clicking btn-add-bq-1 once per question.
    ...
    // Staleness check trimmed to 3s — if Bubble.io replaces the DOM node it happens fast;
    // if it mutates in-place the 3s cap avoids burning time on each of the 5 clicks.
    // Re-appearance uses elementToBeClickable (not visibility) so the next click fires
    // only once the button is truly interactive, not just present in the DOM.
    @Step("Add all 5 business questions")
    ...
    // the fold after 5 business questions are added. behavior:'instant' avoids timing
    // gaps that 'smooth' scrolling introduces between scroll end and click dispatch.
