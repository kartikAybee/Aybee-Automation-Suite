...
public abstract class BasePage {
    ...
    // loading spinner); a page reload reliably brings it up. This waits for ANY of the given landmark
    // locators to be PRESENT in the DOM, and if none appear within timeoutSecs it reloads the page and
    // waits again — up to maxReloads reloads. Returns true once a landmark appears, false if none ever
    // did. Use at every question/step entry point (demographics, consent, form / split-test questions)
    // where a stuck initial load would otherwise time out.
    protected boolean waitForAnyLandmarkElseReload(java.util.List<By> landmarks, int timeoutSecs, int maxReloads) {
        for (int attempt = 0; attempt <= maxReloads; attempt++) {
            try {
                new WebDriverWait(driver, timeoutSecs).until((WebDriver d) -> {
                    for (By by : landmarks) {
                        if (!d.findElements(by).isEmpty()) return true;
                    }
                    return false;
                });
                if (attempt > 0) {
                    System.out.println("[Reload] Landmark appeared after reload " + attempt);
                }
                return true;
            } catch (Exception e) {
                if (attempt < maxReloads) {
                    System.out.println("[Reload] None of " + landmarks + " appeared within " + timeoutSecs
                        + "s — reloading page (reload " + (attempt + 1) + "/" + maxReloads + ")");
    ...
    // Single-landmark convenience wrapper for waitForAnyLandmarkElseReload.
